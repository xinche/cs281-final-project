...
def train_model(segment_id, patient_dicoms_dir, dir_of_snapshots):
    print("model_"+str(segment_id)+" training")
    cur_model = None
...
if __name__ == '__main__':
    ...
    init_model_snapshots_dir('model_snapshots')

    #train_model(50,patient_dicoms_dir,'model_snapshots')
    pool = Pool(processes=10)
    args = ((i, patient_dicoms_dir[:499], "model_snapshots") for i in range(63))
    X = pool.starmap_async(train_model, args)
    pool.close()
    pool.join()
    print(X.get())
